...
$idDetailTransaksi = $_GET['id_transaksi'];
// if($idDetailTransaksi)
// var_dump($idDetailTransaksi).PHP_EOL;


$sqlDataTransaksi = "SELECT * FROM tbl_transaksi WHERE id_transaksi = $idDetailTransaksi";
$data = $koneksi->prepare($sqlDataTransaksi);
$data->execute();
$dataTransaksi = $data->fetch(PDO::FETCH_ASSOC);

// var_dump($dataTransaksi['id_user'] === $_SESSION['id_user']).PHP_EOL;
if($dataTransaksi['id_user'] !== $_SESSION['id_user']){
	header("Location: histori-transaksi.php");
}

$sqlDataTransaksi = "SELECT tbl_user.id_user, tbl_user.nama_lengkap, tbl_user.email, tbl_alamat.alamat_lengkap, tbl_alamat.kota, tbl_alamat.provinsi, tbl_alamat.kode_pos, tbl_alamat.nomor_telepon, tbl_produk.nama_produk, tbl_produk.harga, tbl_produk.ukuran, tbl_kategori.nama_kategori, tbl_transaksi.jumlah_produk, tbl_transaksi.total_pembelian, tbl_transaksi.created_at FROM tbl_transaksi JOIN tbl_user ON tbl_transaksi.id_user = tbl_user.id_user JOIN tbl_alamat ON tbl_alamat.id_alamat = tbl_user.id_alamat JOIN tbl_produk ON tbl_transaksi.id_produk = tbl_produk.id_produk JOIN tbl_kategori ON tbl_produk.id_kategori = tbl_kategori.id_kategori WHERE tbl_transaksi.id_transaksi = $idDetailTransaksi";
